## Hi 👋 I'm Arham Shahid

🎓 Cybersecurity Student  
🌐 Interested in Networking & Blue Team Security  
🛡️ Learning Defensive Security, Network Monitoring & Threat Detection  

---

## 👨‍💻 About Me

- 🎓 Studying Cybersecurity
- 🌐 Strong interest in **Network Forensics**
- 🛡️ Exploring **Blue Team Security**
- 📡 Learning **Digital Forensics (DF)**
- 📚 Currently studying **System investigation, Log Analysis, and Incident Response**

---

## 🧰 Skills & Tools

| Category | Tools / Technologies |
|--------|----------------|
| Networking | TCP/IP, DNS, DHCP |
| Network Analysis | Wireshark |
| Digital Forensics | Basic DF concepts, file system analysis, Autopsy |
| Operating Systems | Linux(Kali, Ubuntu, Rocky), Windows |
| Computer Languages | Python, C#, C++, Java |
| Security Fundamentals | Network Security Basics |

---

## 📜 Certifications

| Certification | Issuer | Status |
|--------------|-------|--------|
| [CCNA: Introduction to Networks]([url](https://www.credly.com/badges/4e680774-9572-436a-a891-cda386c59f63/public_url)) | Cisco | Finished |
| [Red Hat System Administration I (RH124 - RHA) - Ver. 9.3]([url](https://www.credly.com/badges/b92e4d41-7c2a-4902-a7e2-dc07a9ebed45/public_url))
 | Red Hat Academy | Finished |
| [Red Hat System Administration II (RH134 - RHA) - Ver. 10]([url](https://www.credly.com/badges/80cd5d63-687e-478f-8209-dffbdb25e54f/public_url))
 | Red Hat Academy | Finished |
 | CCNA: Switching, Routing, and Wireless Essentials | Cisco | In Progress |

---

## 📚 Currently Learning

- Network Defense Fundamentals
- SIEM Basics
- Security Monitoring
- Log Analysis
- Blue Team Operations

---

## 📂 Featured Projects

| Project | Description |
|-------|-------------|
| Course Recovery System For Educational Instituitions
| Network Design - Multi Branch VLAN Setup
| Smart Retaurant Chef Module
| University Management System

---

## 📫 Connect With Me

- LinkedIn: https://www.linkedin.com/in/muhammad-arham-shahid
- Email: tp084288@mail.apu.edu.my
